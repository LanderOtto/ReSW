...
def plot_tool_single_panel(data, output_dir, name, nfcore_modules):
    ...
    ax_left.set_xticklabels(
        [f"Single-tool\nn={total_single}", f"Multi-tool\nn={total_multi}"], fontsize=16
    )
    ax_left.set_ylabel("Step scripts", fontsize=20)
    ax_left.tick_params(axis="y", labelsize=18)
    ax_left.spines["top"].set_visible(False)

    ax_right.set_ylabel(
        "Step scripts (multi-tool)", fontsize=20, color=RIGHT_AXIS_COLOR
    )
    ax_right.tick_params(axis="y", labelsize=18, colors=RIGHT_AXIS_COLOR)
    ...
    filtered = zip(handles, labels, strict=True)
    leg = ax_right.legend(
